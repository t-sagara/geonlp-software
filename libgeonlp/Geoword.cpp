...
namespace geonlp
{
  ...
  /// JSON からオブジェクトを復元する
  Geoword Geoword::fromJson(const std::string& json_str) throw (picojson::PicojsonException) {
    Geoword in;
...
#endif /* HAVE_LIBDAMS */
    for (prefix_no = 0; prefix_no < prefix.size(); prefix_no++) {
      std::string prefix_str = prefix[prefix_no];
      for (suffix_no = 0; suffix_no < suffix.size(); suffix_no++) {
  ...
  bool Geoword::get_kana_parts_for_surface(const std::string& surface, std::string& prefix_kana, std::string& suffix_kana) const {
    ...
    if (prefix_no >= 0) {
      ...
      if (prefix_no < prefixes.size()) prefix_kana = prefixes[prefix_no];
    }
    if (suffix_no >= 0) {
      std::vector<std::string> suffixes = this->get_suffix_kana();
      if (suffix_no < suffixes.size()) suffix_kana = suffixes[suffix_no];
    }
    return true;
  }
