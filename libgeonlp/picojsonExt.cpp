...
namespace picojson
{
  ...
  picojson::value ext::get_value(const std::string& key) const throw (PicojsonException) {
  ...
  std::string ext::_get_string(const std::string& key) const throw (PicojsonException) {
  ...
  std::vector<std::string> ext::_get_string_list(const std::string& key) const throw (PicojsonException) {
    const picojson::value v0 = this->get_value(key);
    picojson::value v1;
  ...
  std::map<std::string, std::string> ext::_get_string_map(const std::string& key) const throw (PicojsonException) {
    const picojson::value v0 = this->get_value(key);
    picojson::value v1;
  ...
  bool ext::_get_bool(const std::string& key) const throw (PicojsonException) {
    const picojson::value v0 = this->get_value(key);
    if (!v0) return false;
  ...
  int ext::_get_int(const std::string& key) const throw (PicojsonException) {
    const picojson::value v0 = this->get_value(key);
    if (!v0) return 0;
  ...
  std::vector<int> ext::_get_int_list(const std::string& key) const throw (PicojsonException) {
    const picojson::value v0 = this->get_value(key);
    picojson::value v1;
  ...
  // キーが存在しない場合は 0 を返す
  // 値が object, array, string 型の場合は PicojsonException 例外を発生する
  double ext::_get_double(const std::string& key) const throw (PicojsonException) {
    const picojson::value v0 = this->get_value(key);
    if (!v0) return 0.0;
  ...
  std::vector<double> ext::_get_double_list(const std::string& key) const throw (PicojsonException) {
    const picojson::value v0 = this->get_value(key);
    picojson::value v1;
  ...
  ext ext::fromJson(const std::string& json_str) throw (PicojsonException) {
    ext e;
