...
namespace geonlp
{
  ...
  MAImpl::MAImpl( MeCabAdapterPtr mp, DBAccessorPtr dp, DoubleArrayPtr dap, ProfilePtr profilesp) throw (std::runtime_error):
    mecabp(mp), dbap(dp), dap(dap), formatter()
  {
	...
	ss << (*it);
	ss >> v;
	if (this->findDictionaryById(v, dictionary)) this->defaultDictionaries[v] = dictionary;
      }
  ...
  bool MAImpl::findDictionaryById(int dictionary_id, Dictionary& ret) const {
    return this->dbap->findDictionaryById(dictionary_id, ret);
  }
  ...
  void MAImpl::addActiveDictionaries(const std::vector<int>& dics) {
    ...
    for (std::vector<int>::const_iterator it = dics.begin(); it != dics.end(); it++) {
      if (this->findDictionaryById((*it), dictionary)) this->activeDictionaries[(*it)] = dictionary;
    }
  }
  ...
  std::string MAImpl::parse(const std::string & sentence) const
    throw (SqliteNotInitializedException, SqliteErrException, MeCabNotInitializedException, MeCabErrException)
  {
						       ...
						       NodeExtList::iterator& s, NodeExtList::iterator& e) const
  {
    unsigned int len = 0;
    ex = s = e = end;
    // H(head)：地名語の先頭となり得る品詞　あるいは P(prefix):接頭辞　を見つける
    NodeExtList::iterator it;
    for ( it = start; it != end; it++) {
      if ( (*it).canBeHead()) {
	// Hが見つかった
	// std::cout << "found head: " << it->get_surface() << std::endl;
	s = e = it;
