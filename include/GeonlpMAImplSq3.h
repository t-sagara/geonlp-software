...

{
  ...
  public:
    // コンストラクタ
    MAImpl( MeCabAdapterPtr mp, DBAccessorPtr dp, DoubleArrayPtr dap, ProfilePtr p) throw (std::runtime_error);
		
    // デストラクタ
    ~MAImpl();

    /// ID で指定した辞書情報を取得する
    bool findDictionaryById(int dictionary_id, Dictionary& ret) const;
  ...
  private:
    ...
    // 見出し語IDから地名語Nodeを得る。
    Node getGeowordNode(unsigned int id, std::string& alternative) const throw (SqliteNotInitializedException, SqliteErrException);
