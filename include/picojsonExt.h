...
namespace picojson
{
  ...
  public:
    ...
    std::string _get_string(const std::string& key) const throw (PicojsonException);
    // picojson オブジェクトの指定したキーの値を文字列の配列として取得する
    std::vector<std::string> _get_string_list(const std::string& key) const throw (PicojsonException);
    // picojson オブジェクトの指定したキーの値を文字列のハッシュとして取得する
    std::map<std::string, std::string> _get_string_map(const std::string& key) const throw (PicojsonException);
    // picojson オブジェクトの指定したキーの値を整数値として取得する
    int _get_int(const std::string& key) const throw (PicojsonException);
    // picojson オブジェクトの指定したキーの値を整数値の配列として取得する
    std::vector<int> _get_int_list(const std::string& key) const throw (PicojsonException);
    // picojson オブジェクトの指定したキーの値を実数値として取得する
    double _get_double(const std::string& key) const throw (PicojsonException);
    // picojson オブジェクトの指定したキーの値を実数値の配列として取得する
    std::vector<double> _get_double_list(const std::string& key) const throw (PicojsonException);
  ...
  public:
    ...
    picojson::value get_value(const std::string& key) const throw (PicojsonException);

    /// JSON テキストを得る。
    std::string toJson() const;

    /// JSON からオブジェクトを復元する
    static ext fromJson(const std::string& json_str) throw (PicojsonException);
